class DefiniteFormBuilder < ActionView::Helpers::FormBuilder
  ...
  %w[text_field text_area password_field collection_select grouped_collection_select email_field number_field file_field select].each do |method_name|
    alias_method "plain_#{method_name}".to_sym, method_name.to_sym

    define_method(method_name) do |name, *args|
      content_tag(:dt, field_label(name, *args), class: _class(name)) +
      content_tag(:dd, super(name, *jqueryfy(args)), class: _class(name))
    end
  end
  ...
  def time_select(name, *args)
    content_tag(:dt, field_label(name, *args), class: _class(name)) +
    content_tag(:dd, super(name, *jqueryfy(args, :tiny)), class: _class(name))
  end

  def submit(value=nil, *args)
    '</dl><dl class="buttons">'.html_safe + content_tag(:dt, "&nbsp;".html_safe) + content_tag(:dd, super)
  end
...
private
  ...
  def _class(method_name)
    singular = ActiveModel::Naming.param_key(object) rescue 'unknown'
    "fields_for_#{singular}_#{method_name}"
  end

  def jqueryfy(args, jui_size=:normal)
    options = {
      jui: jui_size
    }.merge(args.extract_options!)
  ...
  def field_label(name, *args)
    ...
    required = object.class.validators_on(name).any? {|v|
