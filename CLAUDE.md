...
- **`app/src/layouts/LazyRouteBoundary.tsx` is this codebase's only error
  ...
  files that no longer exist. It recovers by reloading the page (capped via
  a `sessionStorage` timestamp so a genuinely missing chunk can't produce a
  reload loop), and **re-throws every error that isn't a chunk-load
  failure** so ordinary render errors keep behaving as they always have.
